...
import com.devilGhost.ResponseHub.services.DTOValidatorService;
...
public class DashboardController {
    ...
    @GetMapping("/key")
    public ResponseEntity<?> getApiKey(@AuthenticationPrincipal User user) {
        try {
            return dashboardService.getUserApiKey(user.getUsername());
    ...
    public ResponseEntity<?> createFixedEndpoint(
        ...
        try {
            String constraintError = dtoValidatorService.validateFixedRequestDTO(newFixedEndpoint);
    ...
    public ResponseEntity<?> updateFixedEndpoint(
        ...
        try {
            String constraintError = dtoValidatorService.validateFixedRequestDTO(endpointToBeUpdated);
        ...
        try {
            if (method.isBlank() || endpoint.isBlank()) {
                return ResponseEntity
                        .status(HttpStatus.BAD_REQUEST)
                        .contentType(MediaType.APPLICATION_JSON)
                        .body(Map.of("error", "Resource to delete is missing."));
            }
